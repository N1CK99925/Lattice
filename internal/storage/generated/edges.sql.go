...
const deadcode = `-- name: Deadcode :many
SELECT s.id, s.name
FROM symbols s
...
func (q *Queries) Deadcode(ctx context.Context) ([]DeadcodeRow, error) {
	rows, err := q.db.QueryContext(ctx, deadcode)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []DeadcodeRow
	for rows.Next() {
		var i DeadcodeRow
